...
const getLoans = async (req, res) => {
  try {
    ...
    // Compute effective status and penalties for each loan based on current date
    const loansWithEffectiveStatus = loans.map(loan => {
      const loanData = loan.toJSON();
      loanData.penalties = computeEffectivePenalties(loanData);
...
const getLoan = async (req, res) => {
  try {
    ...
    // Compute effective penalties and status based on current date
    const loanData = loan.toJSON();
    loanData.penalties = computeEffectivePenalties(loanData);
    loanData.status = computeEffectiveStatus(loanData);
